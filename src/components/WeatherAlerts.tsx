...
const alertFormSchema = z.object({
  ...
  recommendation: z.string().min(5, { message: 'Recommendation is too short' }),
...
const WeatherAlerts = () => {
  ...
  const [description, setDescription] = useState('Track weather alerts impacting crops and prepare preventive actions');
  ...
  const [weatherAlerts, setWeatherAlerts] = useState<WeatherAlert[]>([
    ...
    },
    {
      ...
      severity: 'Extreme',
      impactCrops: 'Severe',
      description: 'Tropical storm approaching with strong winds and heavy rain.',
      recommendation: 'Harvest mature crops preventively. Reinforce plant supports. Secure farm equipment.',
      status: 'Planned'
    },
  ]);
  ...
  const handleTitleChange = (value: string | number) => {
    ...
    toast({ title: 'Title updated', description: 'The module title has been updated successfully' });
  };
  
  const handleDescriptionChange = (value: string | number) => {
    setDescription(String(value));
    toast({ title: 'Description updated', description: 'The module description has been updated successfully' });
  };
  ...
  const handleTableUpdate = (rowIndex: number, columnId: string, value: any) => {
    ...
    if (dataIndex !== -1) {
      const updatedItem = { ...newData[dataIndex], [columnId]: value } as WeatherAlert;
      newData[dataIndex] = updatedItem;
      setWeatherAlerts(newData);
      toast({ title: 'Alert updated', description: 'The alert information has been updated' });
    }
  };
  ...
  const onSubmit = (data: z.infer<typeof alertFormSchema>) => {
    ...
    setDialogOpen(false);
    form.reset();
    toast({ title: 'Alert added', description: `New weather alert added for ${data.region}` });
  };
      ...
      <div className="bg-white rounded-xl border p-6">
        <div className="mb-6">
          <h2 className="text-xl font-bold flex items-center">
            ...
            <EditableField value={title} onSave={setTitle} className="inline-block" />
          </h2>
          <p className="text-muted-foreground">
            <EditableField value={description} onSave={setDescription} className="inline-block" />
          </p>
        </div>
        
        <div className="flex flex-wrap gap-4 mb-6 items-center">
          <div className="relative flex-grow max-w-sm">
            <Input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search an alert..." className="pl-10" />
            <Filter className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          </div>
          <Select value={filterSeverity} onValueChange={setFilterSeverity}>
            <SelectTrigger className="w-[150px]"><AlertTriangle className="h-4 w-4 mr-2" /><SelectValue placeholder="Severity" /></SelectTrigger>
            <SelectContent>
          ...
          <Select value={filterStatus} onValueChange={setFilterStatus}>
            <SelectTrigger className="w-[150px]"><Calendar className="h-4 w-4 mr-2" /><SelectValue placeholder="Status" /></SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All</SelectItem>
          ...
          <div className="ml-auto">
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogTrigger asChild>
                <Button><PlusCircle className="h-4 w-4 mr-2" />New alert</Button>
              </DialogTrigger>
              <DialogContent className="max-w-lg">
                <DialogHeader><DialogTitle>Add a new weather alert</DialogTitle></DialogHeader>
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <FormField control={form.control} name="severity" render={({ field }) => (
                        <FormItem>
                          ...
                          <Select onValueChange={field.onChange} defaultValue={field.value}>
                            <FormControl><SelectTrigger><SelectValue placeholder="Select a severity" /></SelectTrigger></FormControl>
                            <SelectContent>
                              <SelectItem value="Low">Low</SelectItem>
                              <SelectItem value="Medium">Medium</SelectItem>
                              <SelectItem value="High">High</SelectItem>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <FormField control={form.control} name="impactCrops" render={({ field }) => (
                        <FormItem>
                          <FormLabel>Impact on crops</FormLabel>
                          <Select onValueChange={field.onChange} defaultValue={field.value}>
                            <FormControl><SelectTrigger><SelectValue placeholder="Select an impact" /></SelectTrigger></FormControl>
                            <SelectContent>
                              <SelectItem value="Low">Low</SelectItem>
        ...
        <div className="space-y-4 mb-6">
          {filteredAlerts.length === 0 ? (
            ...
            filteredAlerts.map(alert => (
              <div key={alert.id} className="border rounded-lg overflow-hidden hover:border-blue-200 transition-all">
                <div className="p-4 flex items-center justify-between cursor-pointer" onClick={() => handleExpandAlert(alert.id)}>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {expandedAlertId === alert.id ? (<ArrowUp className="h-4 w-4 text-muted-foreground" />) : (<ArrowDown className="h-4 w-4 text-muted-foreground" />)}
                  </div>
                </div>
                {expandedAlertId === alert.id && (
                  <div className="p-4 bg-muted/20 border-t">
                    ...
                    <div className="mt-4">
                      ...
                      <Badge className={alert.impactCrops === 'Severe' ? 'bg-red-100 text-red-800' : alert.impactCrops === 'Moderate' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'}>
                        {alert.impactCrops}
                      </Badge>
                    </div>
                  </div>
